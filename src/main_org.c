...
int LoadTextLanguage(int new_id)
{
	...
	if(psp_model == 0) sce_paf_private_strcpy(string.options[11], string.slimcolors);
...
{
	AddVshItem = (void *)text_addr + 0x22608;
	ExecuteAction = (void *)text_addr + 0x169A8;
	UnloadModule = (void *)text_addr + 0x16D9C;
	OnXmbPush = (void *)text_addr + 0x168EC;
	OnXmbContextMenu = (void *)text_addr + 0x163A0;
	...
	MAKE_CALL(text_addr + 0x20EBC, AddVshItemPatched);
	MAKE_CALL(text_addr + 0x16984, ExecuteActionPatched);
	MAKE_CALL(text_addr + 0x30828, ExecuteActionPatched);
	MAKE_CALL(text_addr + 0x16B7C, UnloadModulePatched);

	_sw(0x8C48000C, text_addr + 0x5ED4); //lw $t0, 12($v0)
	MAKE_CALL(text_addr + 0x5ED8, OnInitAuthPatched);

	REDIRECT_FUNCTION(text_addr + 0x3F264, scePafGetTextPatched);

	_sw((u32)OnXmbPushPatched, text_addr + 0x52E34);
	_sw((u32)OnXmbContextMenuPatched, text_addr + 0x52E40);
...
{
	AddSysconfItem = (void *)text_addr + 0x2828C;
	GetSysconfItem = (void *)text_addr + 0x23854;
	OnInitMenuPspConfig = (void *)text_addr + 0x1CC08;

	sysconf_unk = text_addr + 0x330F0;
	sysconf_option = text_addr + 0x335BC;
	...
	REDIRECT_FUNCTION(text_addr + 0x299E0, PAF_Resource_GetPageNodeByID_Patched);
	REDIRECT_FUNCTION(text_addr + 0x29A98, PAF_Resource_ResolveRefWString_Patched);
	REDIRECT_FUNCTION(text_addr + 0x29708, scePafGetTextPatched);

	_sw((u32)OnInitMenuPspConfigPatched, text_addr + 0x30424);

	static wchar_t mac_info[] = L"00:00:00:00:00:00";

	if(!config.nothidemacaddress) sce_paf_private_memcpy((void *)text_addr + 0x2E4D8, mac_info, sizeof(mac_info));

	if(psp_model == 0 && config.usbcharge_slimcolors)
	{
		_sw(_lw(text_addr + 0x7664), text_addr + 0x7660);
		_sw(0x24020001, text_addr + 0x7664);
	}
...
{
	MAKE_DUMMY_FUNCTION0(text_addr + 0x202A8); //For POPS exection
	MAKE_DUMMY_FUNCTION0(text_addr + 0x20378); //For POPS resume
	MAKE_DUMMY_FUNCTION0(text_addr + 0x20BC8); //For homebrews

	if(config.hidepic0pic1)
	{
		_sw(0x00601021, text_addr + 0x1D5DC);
		_sw(0x00601021, text_addr + 0x1D5E8);
	}

	if(config.skipgameboot)
	{
		MAKE_CALL(text_addr + 0x18F14, text_addr + 0x19294);
		_sw(0x24040002, text_addr + 0x18F18); //li $a0, 2
	}
...
void PatchUpdatePlugin(u32 text_addr, u32 text_size)
{
	...
	_sw(0x24040000 | (sctrlHENGetVersion() & 0xFFFF), text_addr + 0x81B8);
	...
	for(i = 0; i < text_size; i += 4)
	{
	    ...
	    if(sce_paf_private_strncmp((char *)addr, "/UPDATE", 7) == 0) sce_paf_private_strcpy((char *)addr, "/TN_HEN");
	}
